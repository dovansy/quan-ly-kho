...
const SalesPage = () => {
  ...
  const totalRevenue = useMemo(() => data.reduce((s, x) => s + x.total_amount, 0), [data]);
  const totalDebt = useMemo(
    () => data.filter(x => !x.paid).reduce((s, x) => s + x.total_amount, 0),
    [data]
  );
  ...
  const openEdit = (r: SaleOrderRow) => {
    ...
    const enriched = r.items.map(it => {
      const inv = (inventoryRes?.data || []).find(
        (x: any) =>
  ...
  const onSubmit = () => {
    modalForm.validateFields().then(values => {
      ...
      for (const l of lines) {
        ...
        if (l.quantity <= 0) {
          message.warning(`Số lượng SP "${l.product_name}" phải > 0`);
          return;
        }
        if (l.quantity > l.available) {
          message.warning(
            `Tồn không đủ cho "${l.product_name}" (lô ${l.batch}): còn ${l.available}, cần ${l.quantity}`
          );
          return;
        }
      }
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(
      [
        ...
        { title: 'Khách hàng/Đơn hàng', dataIndex: 'customer_name' },
        ...
        { title: 'Tổng tiền', dataIndex: 'line_total' },
      ],
      flat,
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'center' as const,
      render: (v: boolean) =>
        v ? <Tag color="success">Đã trả</Tag> : <Tag color="error">Còn nợ</Tag>,
    },
    {
      title: 'Ngày bán',
      dataIndex: 'sale_date',
    ...
    },
    {
      ...
      render: (_: any, r: SaleOrderRow) => (
        <ActionColumn
          ...
          deleteDescription={`Xóa hóa đơn "${r.invoice_code}"?`}
        />
      ),
    },
  ];
